    ...
    collector
        .on('collect', async (i) => {
            ...
            if (i.user.id === author.id) {
                ...
                i.followUp({ embeds: [new MessageEmbed().setDescription(`Only ${author} can use this buttons!`)], ephemeral: true });
            }
...
const queuepaginationEmbed = async (interaction, pages, buttonList, author, timeout) => {
    ...
    let page = 0;
    const row = new MessageActionRow().addComponents(buttonList);
    const currentPage = await interaction.followUp({ embeds: [pages[page].setFooter(`Page ${page + 1}/${pages.length}`)], components: [row], fetchReply: true, ephemeral: true  });
