  ...
  if (s.loadType === `${no} LOAD_FAILED`) {
    ...
    return await message.channel.send({
      embeds: [new MessageEmbed() .setColor(client.embedColor)
        .setDescription(`Queued [${s.tracks[0].title}](https://www.youtube.com/watch?v=dQw4w9WgXcQ)  - [\`${message.member.user.tag}\`]`)]
    }).catch(() => {});
    ...
    return await message.channel.send({
      embeds: [new MessageEmbed().setColor(client.embedColor)
        .setDescription(`Queued [${query}](https://www.youtube.com/watch?v=dQw4w9WgXcQ) -  [\`${message.member.user.tag}\`]`)]
    }).catch(() => {});
